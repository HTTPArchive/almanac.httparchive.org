...
chapter_number: 5
title: サードパーティ
description: 2019 Web年鑑のサードパーティの章。サードパーティの使用目的、パフォーマンスへの影響、プライバシーへの影響について説明しています。
...
これらの目標を可能な限り正確に一致させるために、この章で使用されるサードパーティリソースの正式な定義は、HTTPアーカイブデータセット内の少なくとも50のユニークなページにリソースを見つけることができるドメインに由来するリソースです。
...
スクリプトの実行時間の57％はサードパーティ製のスクリプトによるもので、トップ100のドメインはすでにウェブ上のスクリプト実行時間の48％を占めています。このことは、少数のエンティティがウェブのパフォーマンスに与える影響が実際にどれほど大きいかを明確に示しています。このトピックについては、[反響 > パフォーマンス](#performance)セクションで詳しく説明しています。
