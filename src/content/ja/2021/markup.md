...
デスクトップ600万ページのうち、84.4%がgzip（62.7%）またはBrotli（21.7%）で圧縮されていることがわかりました。モバイルページについても、85.6%がgzip（63.7%）またはBrotli（21.9%）で圧縮されており、非常に似たような数字となっています。 {# TODO - make sem edits here #}

モバイルとデスクトップでパーセンテージが若干異なるのは、驚くことではありませんが、驚くべきは、モバイルのみで圧縮されるページがほぼ1ポイント多いということです。 [データの1バイトごとにコストがかかるモバイルの世界](./mobile-web)では、モバイルのページが最適化されているだけでなく、デスクトップよりも小さいことを確認するのは素晴らしいことです。コンテンツエンコーディングの状態やモバイルウェブについては、[圧縮](./compression) や [モバイルウェブ](./mobile-web) の章で詳しく説明しています。 {# TODO - make sem edits here #}
