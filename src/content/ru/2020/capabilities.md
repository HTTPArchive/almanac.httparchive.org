...
{{ figure_markup(
  ...
  caption='Процент загрузок страниц в Chrome, использующих <span lang="en">Async Clipboard API</span>.<br>(источники: <a href="https://chromestatus.com/metrics/feature/timeline/popularity/2369">Async Clipboard Read</a>, <a href="https://chromestatus.com/metrics/feature/timeline/popularity/2370">Async Clipboard Write</a>)',
...
Благодаря <a lang="en" href="https://developer.mozilla.org/ru/docs/Web/API/StorageManager">StorageManager API</a> из [рабочего проекта стандарта WHATWG Storage](https://storage.spec.whatwg.org/#storagemanager), браузеры не будут больше похожи на чёрный ящик. Данный API позволяет разработчикам определить оставшееся доступное место и решить, нужно ли сохранять данные в [постоянное хранилище](https://web.dev/persistent-storage/). В результате появляется возможность предотвратить очистку данных сайта при нехватке дискового пространства. Этот API представлен как новый интерфейс `StorageManager` в объекте `navigator`, который сейчас доступен в браузерах Chrome, Edge и Firefox.
...
{{ figure_markup(
  ...
  caption='Процент загрузок страниц в Chrome, использующих <span lang="en">Notification Triggers API</span>.<br>(источник: <a href="https://chromestatus.com/metrics/feature/timeline/popularity/3017">Notification Triggers</a>)',
...
Для экономии заряда аккумулятора мобильные устройства уменьшают яркость экрана и в конце концов отключают дисплей. Однако есть ситуации, когда при использовании приложения пользователем экран не должен отключаться, например, это актуально при чтении рецепта во время приготовления пищи или при просмотре презентации. <a lang="en" href="https://developer.mozilla.org/en-US/docs/Web/API/Screen_Wake_Lock_API">Screen Wake Lock API</a> ([рабочий черновик W3C](https://www.w3.org/TR/screen-wake-lock/)) решает данную проблему, позволяя оставлять экран включенным.
