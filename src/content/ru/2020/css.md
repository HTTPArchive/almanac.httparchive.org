...
В прошлом единственным решением этих проблем была отдельная таблица стилей с переопределениями для разных систем письма. Однако совсем недавно CSS приобрёл [*логические* свойства и значения](https://developer.mozilla.org/docs/Web/CSS/CSS_Logical_Properties), которые работают так же, как их *физические* аналоги, но чувствительны к направленности их контекста. Например, вместо `width` мы можем написать `inline-size`, а вместо `left` использовать свойство [`inset-inline`](https://developer.mozilla.org/docs/Web/CSS/inset-inline). Помимо логических *свойств*, существуют также логические *ключевые слова*, такие как `float: inline-start` вместо `float: left`.
...
Директива поддержки фичей ([@supports](https://developer.mozilla.org/docs/Web/CSS/@supports)) в течение последних нескольких лет неуклонно набирала обороты и использовалась на 39% страниц, что является заметным увеличением по сравнению с 30% в прошлом году.
...
Почти все свойства `flex`, `flex-*` очень широко применяются, появляясь на 30–60% страниц. Тем не менее, и `flex-wrap`, и `flex-direction` применяются гораздо чаще, чем их сокращение. Свойство `flex-flow`, как правило, применяется с двумя значениями, то есть как более короткий способ задать оба его полных свойства. Несмотря на [тщательно продуманные и здравые значения по умолчанию](https://developer.mozilla.org/docs/Web/CSS/flex#Syntax:~:text=The%20flex%20property%20may%20be%20specified%20using%20one%2C%20two%2C%20or%20three%20values) для использования `flex` с одним или двумя значениями, около 90% применений состоит из синтаксиса с тремя значениями, явно задавая все три его полных свойства.
