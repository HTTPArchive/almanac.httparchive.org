...
featured_stat_label_2: Websites with the attribute <code>aria-labeledby</code>, (misspelling of <code>aria-labelledby</code>)
featured_stat_3: 18
featured_stat_label_3: Average alt text length.
---
...
We found that 20.98% of desktop pages and 21.00% of mobile pages had at least one element with the `aria-expanded` attribute and 17.38% of desktop pages and 16.94% of mobile pages had at least one element with the `aria-controls` attribute. This suggests that around one fifth of websites might be implementing at least partially accessible disclosure widgets. Note that the `aria-controls` attribute is considered a best practice for the disclosure pattern because screen reader support is not ideal.
