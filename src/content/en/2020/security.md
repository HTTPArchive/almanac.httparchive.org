...
Moving our web traffic to HTTPS, and eventually [marking HTTP as non-secure](https://www.chromium.org/Home/chromium-security/marking-http-as-non-secure) is being driven by web browsers restricting [powerful new features to the secure context](https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts/features_restricted_to_secure_contexts) (the carrot) while also increasing warnings shown to users when unencrypted connections are used (the stick).
...
Interestingly, when we look at the most commonly used directives in CSP policies, the most common directive is [`upgrade-insecure-requests`](https://www.w3.org/TR/upgrade-insecure-requests/), which is used to signal to the browser that any content that is included from an insecure scheme should instead be accessed via a secure HTTPS connection to the same host.
