...
Thankfully, there is a way to measure some aspects of performance automatically! We know the browser is in charge of loading a page, and it goes through a checklist of steps each time. Depending on which step the browser is on, we can tell how far along the site is in the page load process. Conveniently, a [number of performance timeline APIs](https://developer.mozilla.org/en-US/docs/Web/API/Performance) are used to fire off timestamps when the browser gets to certain page load steps.
...
As the industry continues to learn more about CWV, we’re seeing steady improvement both in terms of implementation and across all top-level metric values themselves. The most visible performance optimization strides are at the platform level, like Android and bfcache improvements, since the impact can be felt across many sites at once. But let’s look at the most elusive piece of the performance puzzle: individual site owners.

Google’s decision to make CWV part of search ranking catapulted performance to the top of many companies’ roadmaps, and we’re seeing that reflected in the metrics. Site owners are certainly working hard to improve their performance and played a huge role in the CWV improvements over the last year, but those individual efforts are much harder to spot at this scale.

There is still room for improvement like increasing static discoverability of resources (especially LCP elements), and disabling double tap to zoom by including the proper meta viewport tag, which will prevent an artificial 300ms FID delay. INP data uncovers responsiveness problems that FID didn’t show. INP’s higher correlation to TBT suggests a greater emphasis on the consequences of client side JavaScript monopolizing the main thread. It shouldn't matter whether INP becomes a CWV for us to care about delivering fast and responsive experiences, and the data shows that we can be doing better.

There will always be more work to do, which is why the most impactful thing we can do is to continue to invest in making web performance more approachable. In the years to come, let’s emphasize getting web performance knowledge the ‘last mile’ to the site owner.
