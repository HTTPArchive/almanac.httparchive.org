...
Ultimately every resource that goes to make up a web page needs to be stored somewhere. For a website, that often means multiple places, all which bring their own costs and overheads.
...
When you visit a web page for the first time, all the resources that page requests need to be transmitted across the internet from the server to your device.
...
JavaScript is of a special concern here, as it not only needs to be downloaded: it also needs to be parsed and executed.
...
Refer to [JavaScript](./javascript) chapter of the for detailed insights into the internet's favorite monkey paw.
...
The median page (50th percentile) makes 76 requests for desktop page loads, 70 on mobile page loads. At all percentiles the difference between desktop and mobile is minimal.
...
Overall page weight is remarkably close when looking at what is served desktop versus mobile user-agents, although the gap grows slightly in the higher percentile (larger) pages. Given that mobile devices tend to have less local resources and more constrained network capabilities, this is concerning.
...
Videos and other interactive embeds can massively increase the overall weight of a page. Videos by their nature can be large in terms of bytes, but other conent—like social media embeds used to embedding a tweet for example—can bring in a substantial amount of JavaScript and other data to enable these to become interactive.

A good design pattern is the use of facades, a form of lazy-loading. This is basically showing a graphical representation of the element, and not loading the full thing in until required. For example, for a YouTube video, the initial load could be just the poster image for the video, an approach taken by the <a hreflang="en" href="https://github.com/paulirish/lite-youtube-embed">popular lite-youtube-embed</a> library, which changes to the actual full YouTube embed on click. Alternatively, it can even behaving more like traditional image lazy-loading and change when in or near the viewport.
...
The facade pattern doesn't need to be limited to third-party resources either—although these do come with the additional downside of additional lookups and connections—but the approach can work well for large self-hosted resources too.
...
A hefty page weight results in long user wait. The expense of ever-inflating web pages is paid in cost of data-access, cost of devices to meet the technical requirements, and time.
...
While they thankfully are extreme compared to most of the web, the absurd outliers seen in the 100th percentile show the potential for unchecked bloat as new file types and functionalities are introduce to the digital experience.
