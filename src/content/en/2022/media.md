...
That's more than HTML (30 KB), CSS (72 KB), JavaScript (461 KB) and fonts (97 KB) combined.
...
On the vast majority of pages, the most impactful resource is an image. [Largest Contentful Paint](https://web.dev/lcp/) is a web performance metric that identifies the largest element above the fold. Most of the time that element has an image.

{{ figure_markup(
  content="70.2%",
  caption="Percentage of mobile pages whose LCP responsible element has an image. For desktop, it's 80.3%.",
  classes="big-number",
...
It's hard to overstate images' importance to the web. So, what can we say about images on the web?
...
These `<img>`s don't contain any image content at all. Instead, they're being used for two purposes: for layout (as [spacer GIFs](https://en.wikipedia.org/wiki/Spacer_GIF)) or as [tracking beacons](https://en.wikipedia.org/wiki/Web_beacon).

Any newly authored website should use CSS for layout and the [Beacon API](https://developer.mozilla.org/en-US/docs/Web/API/Beacon_API) for tracking. Lots of existing content will use tracking pixels and spacer GIFs forever, but it's disheartening that the desktop number here is unchanged from [last year](https://almanac.httparchive.org/en/2021/media#fig-5), and that the mobile number has only shrunk by a tiny amount. [Old habits](https://developers.facebook.com/docs/meta-pixel/implementation/marketing-api#intialize-img) [die hard](https://spacergif.org/stats/)!
...
[Much like last year](https://almanac.httparchive.org/en/2021/media#aspect-ratios), most images are landscape photos, and there is virtually no difference between the mobile and desktop numbers. Similar to 2021, this feels like a huge missed opportunity. As many-an-Instagrammer knows, portrait photos [render larger on mobile screens](https://uxdesign.cc/the-powerful-interaction-design-of-instagram-stories-47cdeb30e5b6) at full-width than square and landscape images do, and [drive higher engagement](https://www.dashhudson.com/blog/best-picture-format-instagram-dimension). Even when the source material is a landscape, we can and should try to tailor images for mobile screens, using [art direction](https://web.dev/codelab-art-direction/).
...
<figure>
  <table>
    <thead>
      <tr>
        <th>Aspect ratio</th>
        <th>Percentage of images (mobile crawler)</th>
      </tr>
    </thead>
    <tbody>
      <tr>
  ...
  <figcaption>{{ figure_link(caption="A ranked list of the top ten image aspect ratios.", sheets_gid="1668821585", sql_file="top_aspect_ratios.sql") }}</figcaption>
</figure>
...
The default color space on the web is [sRGB](https://en.wikipedia.org/wiki/SRGB). CSS colors are specified in sRGB by default and, unless they're marked otherwise, [browsers assume that the colors in images are sRGB, too](https://imageoptim.com/color-profiles.html).
...
<figure>
  ...
  <figcaption>{{ figure_link(caption="A ranked list of the top twenty ICC colorspace descriptions.", sheets_gid="560546690", sql_file="color_spaces_and_depth.sql") }}</figcaption>
</figure>

Nine out of 10 images on the web are untagged, meaning that if they contain RGB data, it will be interpreted as sRGB. Most of the remainder are explicitly tagged with sRGB or something similar to it: "c2ci", "uRGB", and "c2" are all sRGB variants [designed to be minimal and lightweight](https://github.com/saucecontrol/Compact-ICC-Profiles)). Just a little more than 1% of all of the web's images have been tagged with a wider-than-sRGB gamut. More succinctly, wide-gamut images are currently about as popular on the web as grayscale images ([which account for 1.16% of the web's images](https://docs.google.com/spreadsheets/d/1T5oVAVmcH3sM6R-WwH4ksr2jFtPhuLXs3-iXXoABb3E/edit?pli=1#gid=560546690&range=P5).
...
Most pages have at least one image over 100 KB, and the top 10% have one at least image that weighs almost 1 MB or more.
...
<figure>
  ...
  <figcaption>Typical compression ratios and resulting bits/pixel numbers for bitmap RGB data.</figcaption>
</figure>
...
Most of these numbers are essentially unchanged from [last year](https://almanac.httparchive.org/en/2021/media#bits-per-pixel-by-format).
...
And we again see that the typical WebP is a third lighter, per pixel, than the typical JPEG. This is about what we would expect: [Formal studies](https://developers.google.com/speed/webp/docs/webp_study) (which, vitally, use [matched qualities](https://kornel.ski/en/faircomparison)) have estimated that WebP outperforms JPEG by about that same margin.

The only big mover, when compared to last year, is AVIF. The format dropped from 1.5 bits per pixel last year (less compressed than WebP!), all the way down to an even 1.0. This is a huge reduction, but it wasn't entirely unexpected either. AVIF is a very young format whose encoders have been iterating quickly, and whose adoption is broadening significantly. I expect next year the median AVIF will be even more compressed.

Without looking at the quality side of the lossy-compression/quality tradeoff, it's not possible to conclude from these results alone that AVIF offers the "best" compression of all of the web-compatible formats. But this year we can conclude that in real-world usage, it exhibits the most compression. Pair that conclusion with [in-the-lab results](https://netflixtechblog.com/avif-for-next-generation-image-coding-b1d75675fe4), which suggest it also does good job of preserving quality, and the picture starts looking pretty good (pun intended).

[AVIF's browser support](https://caniuse.com/avif) also took a huge leap this year. All of this is to say, if you're sending bitmap images across the web (as,  you may recall, [99.9% of pages do](TODO link to intro stat)), you should at least consider sending AVIFs.
...
{{ figure_markup(
  content="28.7%",
  caption="Percentage of GIFs which  were animated GIFs on mobile.",
  classes="big-number",
...
I found this surprisingly low. Ever since [PNG achieved universal support in 2006](https://caniuse.com/?search=png), there [hasn't been a good reason to ship a non-animated GIF](https://en.wikipedia.org/wiki/Portable_Network_Graphics#Compared_to_GIF). The word "GIF" has become synonymous with its only justifiable use case: Being a portable, universal format for short, silent, autoplaying, looping animation. One wonders whether all of these non-animated GIFs are legacy content, or whether there is a  significant number  of new, non-animated GIFs being created and published to the web. (I hope not!)
...
Given the vast amount of legacy content on the web, going from zero adoption to usage on a quarter of crawled pages within two years is a remarkable feat, and shows just how much demand there was for native lazy-loading.
...
Images embedded with ``<img>`` elements are supposed to be contentful. Which is to say: they're not just decorative; they should contain something meaningful. According to both [WCAG requirements](https://www.w3.org/WAI/WCAG22/Understanding/non-text-content) and [the HTML spec](https://html.spec.whatwg.org/multipage/images.html#`alt`), all contentful images must have `alt`ernative text, and that `alt`ernative text should usually be supplied by the ``alt`` attribute.
...
This result means that almost half of all `<img>`s are obviously inaccessible. If [the in-depth analysis from this year's accessibility chapter](https://almanac.httparchive.org/en/2022/accessibility#images) **[TODO: confirm this link works]** is any indication, a large chunk of the `<img>`s that do have non-blank `alt` attributes aren't all that accessible, either.
...
A third of pages use `srcset`, but two thirds don't. Given the prevalence of fluid grids within responsive designs in 2022, I suspect there are a lot of pages that aren't using `srcset` that should be.

The `srcset` attribute allows authors to describe resources using one of two descriptors: `x` descriptors that specify which screen density a resource is appropriate for, and `w` descriptors, which instead give the browser the resources' width in pixels. Used in conjunction with the `sizes` attribute, `w` descriptors allow browsers to select a resource appropriate for both fluid layout sizes and variable screen densities.
...
I mentioned earlier that `w` descriptors should be used in conjunction with `sizes` attributes. How well are we using `sizes`? In three words: Not very well.
...
Note: Our crawlers didn't actually load the correct resources, so the numbers here are estimates, based in part on the byte sizes of the incorrect resources, which the crawlers actually did load. However, we estimate that a quarter of desktop pages are loading more than 83 KB of extra image data, based purely on bad `sizes` attributes. That is to say: A better, smaller resource is there for the picking in the `srcset`, but because the `sizes` attribute is so erroneous, the browser doesn't pick it. Additionally, 10% of desktop pages that use sizes load more than a half-megabyte of excess image data because of bad `sizes` attributes!

In the short term, individual developers can and should use [RespImageLint](https://ausi.github.io/respimagelint/) to audit and fix their badly broken `sizes` attributes and prevent this kind of waste.

In the medium term, where possible, the web platform needs to provide better tooling. For many developers, authoring (and maintaining!) accurate `sizes` attributes has proven to be too hard. A proposal that would allow [automatic sizes for lazy-loaded images](https://github.com/whatwg/html/pull/8008) is on the table. (Let's hope it progresses in 2023). [The lazysizes.js library](https://github.com/aFarkas/lazysizes) has already proven the appetite for this sort of solution: [10% of `sizes` attributes currently have the value "auto" before JavaScript runs](https://docs.google.com/spreadsheets/d/1T5oVAVmcH3sM6R-WwH4ksr2jFtPhuLXs3-iXXoABb3E/edit#gid=232511628) and are later rewritten to perfectly accurate values by lazysizes.js, before it lazy-loads the image. Note that, because it relies on lazy-loading, this pattern is *not* appropriate for LCP images, or any image above the fold. For these images, the only way forward for performant responsive loading is a well-authored `sizes` attribute.

#### `<picture>`

The last responsive image feature to land in 2014 was the `<picture>` element. While `srcset` hands browsers a menu of resources to choose from, the `<picture>` element allows authors take charge, giving browsers an explicit set of instructions about which child `<source>` element to load a resource from.
...
Usage is split reasonably evenly. Interestingly, type-switching has closed the gap since [last year](https://almanac.httparchive.org/en/2021/media#fig-23). This may be related to the increasing popularity of next-generation image formats like AVIF and WebP.
...
Throughout this analysis it will be useful to keep in mind the [crawlers' viewports](https://almanac.httparchive.org/en/2022/methodology#webpagetest): The desktop crawler was 1376px-wide,  with a DPR of 1x; the mobile crawler was 360px-wide, with a DPR of 3x.
...
How many images are being delivered from a different domain than the document they're embedded on? A majority of them, including [3.6 percentage points more than last year](https://almanac.httparchive.org/en/2021/media#cross-origin-image-hosts).
...
The fact that a growing majority of images are being delivered across domains underscores [how hard images are to get right](https://css-tricks.com/images-are-hard/), and the benefits of enlisting an [image CDN](https://web.dev/image-cdns/) to handle your media for you.
...
What formats are sites delivering in 2022? MP4, with its [universal support story](https://caniuse.com/mpeg4), is king:
...
But MP4's numbers are a couple ticks down from [last year](https://almanac.httparchive.org/en/2021/media#fig-29), and we continue to see blank extensions (.ts, and .m4s) all delivered when streaming using [HLS](https://en.wikipedia.org/wiki/HTTP_Live_Streaming) or [MPEG-DASH](https://en.wikipedia.org/wiki/Dynamic_Adaptive_Streaming_over_HTTP) are gaining ground.

It's encouraging to see responsive video delivery using adaptive streaming on the rise. At the same time, we look forward to the web platform offering [a simple, declarative solution that doesn't rely on JavaScript](https://github.com/whatwg/html/issues/6363).
...
First, `autoplay` overtook `preload` to become the most popular attribute this year. We also see `playsinline`, `muted`, and `loop` increasing in popularity. Perhaps an increasing number of people are [using the `<video>` element to replace animated GIFs](https://web.dev/replace-gifs-with-videos/)? (If so, good!)

The fact that only 12% of `<video>`s have width attributes and just 0.4% (!) have `height` attributes means that most `<video>` elements are susceptible to the same kinds of [CLS](https://web.dev/cls/) problems we saw with `<img>` elements, which lack these attributes.  Help the browser help you and add these attributes!
...
Why? I like to think it's authors getting out of the browser's way.
