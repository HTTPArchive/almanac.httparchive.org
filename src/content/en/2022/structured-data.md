...
_**Editors: Jasmine Drudge-Willson ([jasmine.a.willson@gmail.com](mailto:jasmine.a.willson@gmail.com))**_


# Introduction

This is the second year that the Web Almanac has included a chapter on structured data. Last year's content [gave solid grounding](https://almanac.httparchive.org/en/2021/structured-data#key-concepts) in the concept of structured data, outlining the reason it exists, the most frequently used types, and how it benefits organizations. This year we compared data gathered in 2021 with data gathered in 2022, so were able to monitor trends that occurred within that period.
...
{{ figure_markup(
  image="structureddata-images/image2",
...
_Figure 1 - distribution of structured data types, 2022_


{{ figure_markup(
  image="structureddata-images/image3.png",
...
_Table 1 - changes between 2021 and 2022 in data type formats._
...
To be able to retrieve and manipulate RDF data, we can use an RDF Query Language such as [SPARQL](https://www.w3.org/TR/sparql11-query/), the standard RDF query language.
...
Open Data is the first of the [5 stars of open data](https://5stardata.info/en/), a deployment scheme suggested by Tim Berners-Lee. According to the [open data handbook](https://opendatahandbook.org/), to score the maximum five stars, data must (1) Be available on the Web under an open license, (2) Be in the form of structured data, (3) Be in a non-proprietary file format, (4) Use URIs as its identifiers, (5) Include links to other data sources (see [data linking](#heading=h.u0uq7llkr1cj)).
...
A semantic search engine is one which performs [semantic search](https://en.wikipedia.org/wiki/Semantic_search). This is different from lexical search where search engines look for exact or close matches to words or strings of text. Semantic search aims to understand the user’s intent and the context of the search terms in order to improve the accuracy of search. An example would be a structured data entity of ‘local business: hairdresser’ versus ‘TG Locks n Lashes’; the latter is a business name, and while it tells the creative name of the hair salon as a key-word, it does little to help the search engine to understand what the business does. By using structured data, the website can better help the search engine understand the context of its information, and thus enable the engine to offer better search results in the context of the query asked by the search user. Google and Bing are excellent examples of semantic search engines.

Google uses semantic search technologies to serve relevant information from the [Google Knowledge Graph](https://blog.google/products/search/introducing-knowledge-graph-things-not/) which is a knowledge base used to serve search results in an infobox. This infobox is known as a [knowledge panel](https://support.google.com/knowledgepanel/answer/9163198), and can be seen in many results. This is enabled by structured data.

Another search result that is made possible by structured data combined with linked data is the [rich result](https://developers.google.com/search/docs/advanced/structured-data/search-gallery). These results display richer features in search results, and come in the form of Events, FAQs, How-tos, Job listings and [many more](https://developers.google.com/search/docs/advanced/structured-data/search-gallery). Implementing structured data to make web pages eligible for rich results [could increase clickthrough rate](https://www.searchenginejournal.com/how-important-is-structured-data/257775/). Image 1 illustrates how structured data with business details for a Hair Studio allows the search engine to easily extract and display information about the business, highlighting it and optimizing SEO.


![alt_text](images/image4.png "image_tooltip")


_Image 1 - Structured data surfaced in a web search_

Beyond knowledge panels and web page rich results, structured data can also enable answers to [factual queries](https://gofishdigital.com/blog/answering-questions-structured-data/) in search.  A factual query search can get multiple signals from different structured data sources and support more precise results. Here, structured data implementation, and the technologies that allow for it, provide faster and more reliable access to information in order to improve user experience.
...
WikiData - [https://en.wikipedia.org/wiki/Wikidata](https://en.wikipedia.org/wiki/Wikidata)
...
{{ figure_markup(
  image="structureddata-images/image6.png",
...
{{ figure_markup(
  image="structureddata-images/image8.png",
...
_Figure 6 - Distribution of RDFa usage, 2021 Vs 2022_
...
{{ figure_markup(
  image="structureddata-images/image9.png",
...
_Figure 7 - RDFa usage on mobile pages_


{{ figure_markup(
  image="structureddata-images/image10.png",
...
{{ figure_markup(
  image="structureddata-images/image11.png",
...
{{ figure_markup(
  image="structureddata-images/image12.png",
...
_Figure 10 - Dublin Core usage on mobile pages_


{{ figure_markup(
  image="structureddata-images/image13.png",
...
_Figure 11 - Dublin Core usage on desktop pages_
...
{{ figure_markup(
  image="structureddata-images/image14.png",
...
_Figure 12 - Distribution of Open Graph usage, 2021 Vs 2022_
...
{{ figure_markup(
  image="structureddata-images/image15.png",
...
_Figure 13 - Open Graph usage on mobile pages_


{{ figure_markup(
  image="structureddata-images/image16.png",
...
{{ figure_markup(
  image="structureddata-images/image17.png",
...
{{ figure_markup(
  image="structureddata-images/image18.png",
...
_Figure 16 - Twitter meta tag usage on mobile pages_


{{ figure_markup(
  image="structureddata-images/image19.png",
...
{{ figure_markup(
  image="structureddata-images/image20.png",
...
_Figure 18 - Distribution of Facebook meta tag usage, 2021 Vs 2022_
...
{{ figure_markup(
  image="structureddata-images/image21.png",
...
_Figure 19 - Facebook meta tag usage on mobile pages_


{{ figure_markup(
  image="structureddata-images/image22.png",
...
_Figure 20 - Facebook meta tag usage on desktop pages_
...
{{ figure_markup(
  image="structureddata-images/image23.png",
...
{{ figure_markup(
  image="structureddata-images/image24.png",
...
_Figure 22 - Microformats usage on mobile pages_


{{ figure_markup(
  image="structureddata-images/image25.png",
...
_Figure 23 - Microformats usage on desktop pages_
...
{{ figure_markup(
  image="structureddata-images/image26.png",
...
_Figure 25 - Microformats2 usage on mobile pages_


{{ figure_markup(
  image="structureddata-images/image28.png",
...
{{ figure_markup(
  image="structureddata-images/image29.png",
...
{{ figure_markup(
  image="structureddata-images/image30.png",
...
_Figure 28 - Microdata usage on mobile pages_


{{ figure_markup(
  image="structureddata-images/image31.png",
 ...
 _Figure 29 - Microdata usage on desktop pages_
...
{{ figure_markup(
  image="structureddata-images/image32.png",
...
{{ figure_markup(
  image="structureddata-images/image33.png",
...
_Figure 31 - JSON-LD usage on mobile pages_


{{ figure_markup(
  image="structureddata-images/image34.png",
...
_Figure 32 - JSON-LD usage on desktop pages_
...
{{ figure_markup(
  image="structureddata-images/image35.png",
...
{{ figure_markup(
  image="structureddata-images/image36.png",
...
_Figure 34 - SameAs usage on mobile pages_


{{ figure_markup(
  image="structureddata-images/image37.png",
...
_Figure 35 - SameAs usage on desktop pages_
...
{{ figure_markup(
  image="structureddata-images/image38.png",
...
{{ figure_markup(
  image="structureddata-images/image39.png",
...
{{ figure_markup(
  image="structureddata-images/image40.png",
...
_Figure 38 - Relative change of JSON-LD contexts from 2021 to 2022_
...
Search engine optimization ([SEO](https://www.webopedia.com/definitions/seo/#How_does_SEO_work)) is the area focusing on building the content of a web page so that it has better results from search engines. Naturally, this is highly important for discoverability as a successful implementation of SEO may allow for a page to appear higher on the search engine results page ([SERP](https://www.webopedia.com/definitions/serp/)). The SERP is where the titles, URLs, and meta descriptions are displayed from a search query.
...
[Product](https://developers.google.com/search/docs/advanced/structured-data/product), the structured data type, works similarly to LocalBusiness in that it allows for a search query to return rich results. These results can include price, availability, reviews, ratings, and even images in the search results. These added elements can make the product far more likely to receive attention from the search. Product attributes can help link products together and better respond to search queries, increasing discoverability.

These are just a couple of examples of use cases for structured data in ecommerce, but there are many [more structured data types](https://developers.google.com/search/docs/advanced/ecommerce/include-structured-data-relevant-to-ecommerce) that an ecommerce page can benefit from implementing.
