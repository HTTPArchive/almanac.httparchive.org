...
Looking at total image bytes shows us that this metric has remained virtually [https://almanac.httparchive.org/en/2020/page-weight#file-formats](unchanged from the previous year). One reason for this could be an increase in the number images being served by content distribution networks (CDN), which apply strong optimizations to images as they are uploaded to their servers for use on future requests.
