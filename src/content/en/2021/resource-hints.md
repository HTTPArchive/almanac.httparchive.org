...
The most widely adopted resource hints are implemented through the Link directive's rel attribute. These are [`dns-prefetch`](https://www.w3.org/TR/resource-hints/#dfn-dns-prefetch), [`preconnect`](https://www.w3.org/TR/resource-hints/#dfn-preconnect), [`prefetch`](https://www.w3.org/TR/resource-hints/#dfn-prefetch), [`prerender`](https://www.w3.org/TR/resource-hints/#dfn-prerender) and [`preload`](https://www.w3.org/TR/preload/).
...
Using our current methodology, it is not possible to reliably measure resource hints which are added following user-interaction, such as [QuickLink](https://github.com/GoogleChromeLabs/quicklink), which featured on less than 0.1% of pages analyzed.
...
The `dns-prefetch` link initiates an early request to resolve a domain name. It is only effective for DNS lookups on cross-origin domains and may be paired together with `preconnect`. While Chrome now supports a maximum of [64](https://source.chromium.org/chromium/chromium/src/+/fdf9418d23d434e0f7134da67dc41b0fe8268e91:net/dns/host_resolver_manager.cc;l=416) concurrent in-flight DNS requests - up from 6 last year - other browsers still have tighter limitations. For example it is limited to [8](https://github.com/mozilla/gecko-dev/blob/master/netwerk/dns/nsHostResolver.h#L48) on Firefox.
...
Most modern browsers also [support](https://caniuse.com/link-rel-preload) the `preload` hint - and to a [lesser degree](https://caniuse.com/link-rel-modulepreload), the [`modulepreload`](https://html.spec.whatwg.org/multipage/links.html#link-type-modulepreload) hint. The `preload` instruction initiates an early fetch for a resource which is required in the loading of a page and is most commonly used for late-discovered resources, such as font files or images referenced in stylesheets. Preloading a resource may also be used to elevate its priority, allowing the developer to prioritize the loading of an LCP ([Largest Contentful Paint](https://web.dev/lcp)) image, even if this would otherwise be discovered while parsing the HTML.

`modulepreload` is a specialized alternative to `preload` and behaves similarly, however its usage is limited to [module scripts](https://html.spec.whatwg.org/multipage/webappapis.html#module-script).
...
The most widely used resource hint is `dns-prefetch` (36.43% on mobile) - which is unsurprising, considering it was introduced in [2009](https://caniuse.com/link-rel-dns-prefetch) - however with the widespread use of HTTPS, in many cases it should be replaced with `preconnect` (12.72% on mobile). When considering that the `preload` hint is comparatively new - first appearing in Chrome in [2016](https://groups.google.com/a/chromium.org/g/blink-dev/c/_nu6HlbNQfo/m/XzaLNb1bBgAJ?pli=1), it is the second most widely adopted resource hint (22.10% on mobile) and is seeing constant growth year-on-year - a testament to the importance and flexibility of this directive.
...
This same observation may be seen on a page's [Largest Contentful Paint](https://web.dev/lcp) and [First Contentful Paint](https://web.dev/fcp), indicating that in many cases, the developer is prioritizing resources which aren't needed to render the LCP element and as a consequence degrading the user experience.
...
**style**: Stylesheets are ordinarily embedded in the document's `<head>` and discovered early during the document parsing. Additionally, as stylesheets are render-blocking resources they are assigned the _Highest_ request priority. This should make preloading stylesheets unnecessary but it is sometimes required to re-prioritize the requests. A [bug](https://bugs.chromium.org/p/chromium/issues/detail?id=629420) in Google Chrome (fixed in Chrome 95) prioritizes preloaded resources ahead of other higher-priority resources discovered by the preload scanner, including CSS files. Preloading the stylesheet will restore its _Highest_ priority. Another instance when stylesheets are preloaded is when they are not downloaded directly from the HTML document, such as the [asynchronous CSS](https://www.filamentgroup.com/lab/async-css.html) "hack" which uses an `onload` event to avoid render-blocking the page with non-critical CSS.
...
**anonymous**: The default value when no value is specified, `anonymous` will set the credentials flag to [`same-origin`](https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy) and is required when downloading resources protected by CORS. It is also a [requirement](https://drafts.csswg.org/css-fonts/#font-fetching-requirements) when downloading font files - even if they are on the same origin! If you omit the `crossorigin` attribute when the eventual request for the preloaded resource uses CORS, you will end up with a duplicate request since it won't match in the preload cache.
...
`loading="lazy"` on images is supported on most major browsers. On Safari, it is marked as [in progress](https://bugs.webkit.org/show_bug.cgi?id=200764) and is available behind a flag, but not yet enabled by default. Lazy-loading of iframes has similar support, although it is [not yet supported on Firefox](https://bugzilla.mozilla.org/show_bug.cgi?id=1622090).

Browsers which do not support the loading attribute, will simply ignore it - making it safe to add without unwanted side-effects. JavaScript based alternatives, such as [lazysizes](https://github.com/aFarkas/lazysizes) may still be used, however considering that full browser support is around the corner, it may not be worth adding to a project at this stage.
...
So while the technology is widely available, overcoming these obstacles makes it highly unpopular - with less than 1% adoption on Chrome. Chrome has also filed an [Intent to Remove](https://lists.w3.org/Archives/Public/ietf-http-wg/2019JulSep/0078.html) that is paused until a testable implementation of 103 Early Hints is available.

### Future

While there are no proposals to add new rel directives, improvements from the browser vendors to the current set of resource hints - such as the prioritization [bug](https://bugs.chromium.org/p/chromium/issues/detail?id=629420) in Chrome - are expected to have a positive impact. Hint adoption is expected to evolve and the use of `preload` should shift towards its intended purpose - late discovered resources.
