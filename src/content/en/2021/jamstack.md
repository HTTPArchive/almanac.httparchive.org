...
For more complex sites, data has to be structured. There are several ways to store and manage data using headless CMSs using API.

The goal of this chapter is to identify what’s the main SSGs used on Jamstack. How they are distributed around the world, what is the performance of Jamstack sites and how it is growing.
...
Most of the data is represented for mobile and desktop websites. There's a fully described [​​methodology](https://almanac.httparchive.org/en/2021/methodology) used in HTTPArchive.
...
Note: GitHub as CDN provider might be doubted, but GitHub offers a feature called [GitHub Pages](https://pages.github.com/) which allows users to deploy sites on github.io with subdomain using built in Jekyll SSG.
...
Fair to say that Cloudflare is used no only for pure serving as CDN but as protection service, DNS balancing etc. So I can’t 100% say that Cloudflare is .
...
In overall assessment for SSGs we can understand the basic performance level of Jamstack sites. CVW assessment contains data of 75% percentile of page loads  which have a good score of CVW across all metrics.
...
Next generation of image formats - [WebP](https://developers.google.com/speed/webp/) has 17% of usage among Jamtask sites.
...
Data of resource weights confirms that performance of Next.js, Nuxt.js and gatsby is struggling because of huge resources. 2MB of page weight for and ~700Kb of  Javascript can definitely impact performance scores and results, especially for avarage mobile devices and slow networks. Also heavy usage of styled-components might be the cause (there are several researches in areas of performance downgrades), Next.js and Gatsby sites use it really often. Good signal is that Image adoption of next gen image formats is growing. It will improve UX for end users in the long run.
