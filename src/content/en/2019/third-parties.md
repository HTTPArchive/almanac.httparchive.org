...
[Reuse in computer technologies](https://en.wikipedia.org/wiki/Code_reuse) is generally a good thing, though it does introduce depencies which can impact performance (in positive and negative ways!) and has privacy implications, both of which we have touched on here. We have also not discussed the security implications of adding such dependencies to your site, though the security chapter has a [section on vulnerable JS libraries](./security#vulnerable-js-libraries) which is relevant and may be of interest to readers. Even if code has no security concerns it can introduce a single point of failure [which has been recognised as a potenital problem for some time now](https://www.stevesouders.com/blog/2010/06/01/frontend-spof/).
