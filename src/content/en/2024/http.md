...
When we [last looked at HTTP/3 support](https://almanac.httparchive.org/en/2022/http#http3-support) in June 2022, the protocol wasn't even fully standardized yet! Still, due to early deployments from some large players, around 18% of sites in the HTTP Archive dataset indicated they had support for HTTP/3. Now, two years later, we can see that support for the new protocol has steadily risen, up to 26% (desktop) to 28% (mobile), a near 10% overall increase. If that still seems low, it's actually quite similar to HTTP/2's evolution, which equally saw [around 30% uptake in its second year](https://httparchive.org/reports/state-of-the-web#h2) after standardization (2017).
