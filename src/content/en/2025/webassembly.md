...
- **Data Collection:** Data Collection: This chapter relies on this dataset provided by HTTP Archive July 2025 crawl data which is hosted on Google BigQuery. To identify WebAssembly modules by matching the Content-Type (application/wasm) and the .wasm file extension.

- **Analysis:** In addition to the HTTP Archive dataset, we use the <a hreflang="en" href="https://github.com/nimeshgit/almanac-wasm-stats">almanac-wasm-stats</a> tool to download and validate the WebAssembly modules identified, extracts metadata from these downloaded files, allowing us to identify programming languages, libraries, and specific features used within the Wasm modules from the HTTP Archive for local analysis. For language usage stats, we identified 64.3% of desktop and 72.8% of mobile Wasm modules, the rest Wasm modules fall under failures when downloading wasm file and validation.
