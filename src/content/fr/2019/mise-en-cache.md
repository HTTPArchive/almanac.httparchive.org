...
La mise en cache est une technique qui permet de réutiliser un contenu précédemment téléchargé. Il offre un avantage significatif en termes de [performance] (./performance) en évitant de rejouer les requêtes réseau coûteuses. Il aide a la maitrise de la monté en charge d'une application en réduisant le trafic vers l'infrastructure d'origine d'un site Web. Un vieux dicton qui dit que " la requête la plus rapide est celle que vous n'avez pas à faire " et la mise en cache est l'un des principaux moyens d'éviter d'avoir à faire des requêtes.

Il y a trois principes fondamentaux a la mise en cache de contenus Web : mettre en cache autant que possible, aussi longtemps que possible, aussi près que possible des utilisateurs finaux.

**Mettre en cache autant que vous le pouvez.** Lorsque l'on regarde la quantité de données cachable, il est important de comprendre si une réponse est statique ou dynamique. Les requêtes qui sont servies en tant que réponse statique peuvent généralement être mises en cache, car elles ont une relation one-to-many entre la ressource et les utilisateurs. Le contenu dynamique necessite quand a lui une analyse plus poussée.

**Mettre en cache aussi longtemps que possible.** La durée de mise en cache d'une ressource dépend fortement de sa sensibilité de son contenu. Une ressource JavaScript versionnée peut être mise en cache pendant très longtemps, alors qu'une ressource non versionnée peut nécessiter une durée de cache plus courte pour garantir aux utilisateurs de disposer des données a jour.

**Cachez le plus près possible des utilisateurs finaux.** La mise en cache proche des utilisateurs finaux réduit les temps de téléchargement en réduisant les latences. Par exemple, si une ressource est mise en cache sur le navigateur d'un utilisateur, la requête n'est jamais envoyée sur le réseau et le temps de téléchargement est aussi rapide que les I/O de la machine.
Pour les premiers visiteurs, ou les visiteurs qui n'ont pas d'entrées dans leur cache, un CDN est généralement le prochain endroit d'où une ressource en cache est retournée. Dans la plupart des cas, il sera plus rapide de récupérer une ressource à partir d'un cache local ou d'un CDN par rapport à un serveur d'origine.
