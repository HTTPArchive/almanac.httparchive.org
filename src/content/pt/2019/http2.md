...
Em 2009, a Google anunciou que estava trabalhando em uma alternativa ao HTTP baseado em texto chamada [SPDY](https://www.chromium.org/spdy), que já foi descontinuada. Isso tiraria vantagem do fato de que as mensagens HTTP eram frequentemente criptografadas em HTTPS, o que evita que sejam lidas ou sofram interferência no trajeto.
...
O HTTP/2, contudo, era diferente, visto que estava efetivamente oculto em HTTPS (pelo menos para os casos de uso do navegador), removendo as barreiras para adoção, desde que o navegador e o servidor o suportassem. O suporte do navegador tem sido muito forte há algum tempo e o advento da atualização automática nos navegadores, *evergreen*, significou que cerca de [95% dos usuários globais agora suportam HTTP/2](https://caniuse.com/#feat=http2).
...
{{ figure_markup(
  ...
  caption='Uso de HTTP/2 por requisição. (Fonte: <a href="https://httparchive.org/reports/state-of-the-web#h2">HTTP Archive</a>)',
...
No entanto, nossas descobertas são corroboradas por outras fontes, como [telemetria da Mozilla](https://telemetry.mozilla.org/new-pipeline/dist.html#!cumulative=0&measure=HTTP_RESPONSE_VERSION), que analisa o uso em cenário real através do navegador Firefox.
...
{{ figure_markup(
  ...
  caption='Conexões TCP por página. (Fonte: <a href="https://httparchive.org/reports/state-of-the-web#tcp">HTTP Archive</a>)',
...
{{ figure_markup(
  ...
  caption='Requisições totais por página. (Fonte: <a href="https://httparchive.org/reports/state-of-the-web#reqTotal">HTTP Archive</a>)',
