...
if [ "${production}" == "1" ]; then
    ...
    CHANGED_FILES=$(git diff --diff-filter=AM --no-commit-id --name-only main content templates | grep -v base.html | grep -v ejs | grep -v base_ | grep -v toc.html | grep -v sitemap)
