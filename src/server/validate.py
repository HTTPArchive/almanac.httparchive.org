...
def validate_chapter(chapter, year):
    ...
    if chapter not in chapters_for_year:
        if chapter[-1] == "/":
            ...
            logging.debug(
                "Typo chapter requested: %s, redirecting to %s"
                % (chapter, TYPO_CHAPTERS.get(chapter))
            )
...
def validate_lang_and_year(lang, year):
    ...
    if lang is not None and lang not in supported_langs:
        ...
        if (
            lang.lower() in LANGUAGE_MAPPING
            and LANGUAGE_MAPPING.get(lang.lower()) in supported_langs
        ):
