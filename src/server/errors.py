...
def bad_request(e):  # pragma: no cover
    logging.exception(
        "An error occurred during a request due to bad request error: %s", request.path
    )
...
def handle_internal_server_error(e):  # pragma: no cover
    logging.exception(
        "An error occurred during a request due to internal server error: %s",
        request.path,
    )
...
def handle_bad_gateway(e):  # pragma: no cover
    logging.exception(
        "An error occurred during a request due to bad gateway: %s", request.path
    )
