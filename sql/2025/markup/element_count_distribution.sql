CREATE TEMPORARY FUNCTION get_element_count_info(element_count JSON)
RETURNS STRUCT<elements_count INT64, types_count INT64> LANGUAGE js AS '''
var result = {};
if (!element_count) return result;
...
FROM (
  SELECT
    ...
    percentile,
    page,
    get_element_count_info(custom_metrics.element_count) AS element_count_info
  FROM
