CREATE TEMPORARY FUNCTION get_element_types_info(element_count JSON)
RETURNS ARRAY<STRUCT<name STRING, freq INT64>> LANGUAGE js AS '''
if (!element_count) return []; // 2019 had a few cases
...
FROM
  `httparchive.crawl.pages`,
  UNNEST(get_element_types_info(custom_metrics.element_count)) AS element_type_info
JOIN
  totals
USING (client)
WHERE
