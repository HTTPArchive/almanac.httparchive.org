CREATE TEMPORARY FUNCTION getFaviconImage(linknodes JSON)
RETURNS STRUCT<
  image_type_extension STRING
> LANGUAGE js AS '''
var result = {};

if (linknodes && linknodes.nodes && linknodes.nodes.find) {
  var faviconNode = linknodes.nodes.find(n => n.rel && n.rel.split(' ').find(r => r.trim().toLowerCase() == 'icon'));

  if (faviconNode) {
    if (faviconNode.href) {
      var temp = faviconNode.href;

      if (temp.includes('?')) {
        temp = temp.substring(0, temp.indexOf('?'));
      }

      if (temp.includes('.')) {
        temp = temp.substring(temp.lastIndexOf('.')+1);

        result.image_type_extension = temp.toLowerCase().trim();
      }
      else {
        result.image_type_extension = "NO_EXTENSION";
      }

    } else {
      result.image_type_extension = "NO_HREF";
    }
  } else {
    result.image_type_extension = "NO_ICON";
  }
}
else {
  result.image_type_extension = "NO_DATA";
}
return result;
''';

SELECT
  client,
  favicon.image_type_extension AS image_type_extension,
  COUNT(0) AS freq,
  SUM(COUNT(0)) OVER (PARTITION BY client) AS total,
  COUNT(0) / SUM(COUNT(0)) OVER (PARTITION BY client) AS pct
FROM
  (
    SELECT
      client,
      getFaviconImage(custom_metrics.other.almanac.`link-nodes`) AS favicon
    FROM
      `httparchive.crawl.pages`
    WHERE
      date = '2025-07-01'
  )
GROUP BY
  client,
  image_type_extension
ORDER BY
  pct DESC,
  client,
  freq DESC
LIMIT 1000
