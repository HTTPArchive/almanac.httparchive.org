CREATE TEMPORARY FUNCTION get_markup_inputs_info(markup JSON)
RETURNS ARRAY<STRUCT<name STRING, freq INT64>> LANGUAGE js AS '''
var result = [];

if (markup && markup.inputs && markup.inputs.types) {
...
JOIN
  totals
USING (client),
  UNNEST(get_markup_inputs_info(custom_metrics.markup)) AS markup_input_info
WHERE
  date = '2025-07-01'
GROUP BY
  client,
