...
SELECT
  ...
  client,
  rank_grouping,
  CASE WHEN rank_grouping = 10000000 THEN 'all' ELSE FORMAT("%'d", rank_grouping) END AS ranking,
  COUNT(0) AS freq,
  total,
...
FROM (
  ...
  WHERE
    date >= DATE '2022-05-01' AND is_root_page AND
...
JOIN (
  ...
  FROM `httparchive.crawl.pages`,
    UNNEST([1000, 10000, 100000, 1000000, 10000000]) AS rank_grouping
  WHERE rank <= rank_grouping AND is_root_page
  GROUP BY date, rank_grouping, client
) totals
