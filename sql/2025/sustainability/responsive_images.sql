...
SELECT
  client,
  round(
    safe_divide(countif(media_info.num_srcset_all > 0), count(0)) * 100, 2
  ) AS pages_with_srcset_pct,
  round(
    safe_divide(countif(media_info.num_srcset_sizes > 0), count(0)) * 100, 2
  ...
  round(
    safe_divide(countif(media_info.num_picture_img > 0), count(0)) * 100, 2
