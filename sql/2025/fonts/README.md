...
The queries can be executed using the `execute.py` script. To this end, install
...
The script can be run for all or some specific queries as illustrated below:
...
By default, it operates in a dry-run mode: it does not actually run the queries
but prints an estimate on the amount of data that would be processed for each
query. To disable the dry-run mode and actually run the queries, pass the
