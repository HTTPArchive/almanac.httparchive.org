SELECT
  ...
  COUNT(0) AS total,
  COUNTIF(SAFE_CAST(JSON_VALUE(report, '$.audits.unminified-css.score') AS FLOAT64) >= 0.9) / COUNT(0) AS pct_pass,
  COUNTIF(SAFE_CAST(JSON_VALUE(report, '$.audits.unminified-css.score') AS FLOAT64) < 0.9) / COUNT(0) AS pct_fail
FROM
