#standardSQL
# Prevalence of /.well-known/resource-that-should-not-exist-whose-status-code-should-not-be-200 counts for 200 HTTP status codes
...
SELECT
  ...
  SAFE_DIVIDE(COUNTIF(status = 200), COUNT(DISTINCT page)) AS pct_status_200,
  COUNTIF(status != 200) AS count_status_not_ok,
  SAFE_DIVIDE(COUNTIF(status != 200), COUNT(DISTINCT page)) AS pct_status_not_ok
FROM (
    SELECT
