...
SELECT
  client,
  ROUND(SAFE_DIVIDE(COUNTIF(media_info.num_picture_using_orientation > 0) * 100, COUNTIF(media_info.num_picture_img > 0)), 2) AS pages_with_picture_orientation_pct,
  ROUND(SAFE_DIVIDE(SUM(media_info.num_picture_using_orientation) * 100, SUM(media_info.num_picture_img)), 2) AS occurences_of_picture_orientation_pct
FROM
  (
  SELECT
