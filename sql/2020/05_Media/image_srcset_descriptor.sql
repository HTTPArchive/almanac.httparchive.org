...
SELECT
  ...
  ROUND(SAFE_DIVIDE(COUNTIF(media_info.num_srcset_descriptor_x > 0) * 100, COUNT(0)), 2) AS pages_with_srcset_descriptor_x_pct,
  ROUND(SAFE_DIVIDE(COUNTIF(media_info.num_srcset_descriptor_w > 0) * 100, COUNT(0)), 2) AS pages_with_srcset_descriptor_w_pct,
  ROUND(SAFE_DIVIDE(SUM(media_info.num_srcset_descriptor_x) * 100, SUM(media_info.num_srcset_all)), 2) AS instances_of_srcset_descriptor_x_pct,
  ROUND(SAFE_DIVIDE(SUM(media_info.num_srcset_descriptor_w) * 100, SUM(media_info.num_srcset_all)), 2) AS instances_of_srcset_descriptor_w_pct
FROM
  (
  SELECT
