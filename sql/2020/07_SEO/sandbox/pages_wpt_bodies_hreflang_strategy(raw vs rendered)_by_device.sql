...
if (Math.floor(Math.random() * 50) == 0) {
    ...
    if (wpt_bodies.hreflangs) {

      if (wpt_bodies.canonicals.rendered.values.length) {
        result.has_rendered_hreflang = true;
      } else {
        result.has_rendered_hreflang = false;
      }

      if (wpt_bodies.canonicals.raw.values.length) {
        result.has_raw_hreflang = true;
      } else {
        result.has_raw_hreflang = false;
      }

       if (wpt_bodies.canonicals.raw.values.length && wpt_bodies.canonicals.rendered.values.length ) {
        result.has_both_hreflangs = true;
      } else {
        result.has_both_hreflangs = false;
      }
    }
...
# RAW hreflang
# COUNTIF(wpt_bodies_info.robots_has_raw_hreflang) AS freq_has_raw_hreflang,
AS_PERCENT(COUNTIF(wpt_bodies_info.has_raw_hreflang), COUNT(0)) AS pct_has_raw_hreflang,

# Rendered hreflang
# COUNTIF(wpt_bodies_info.robots_has_rendered_hreflang) AS freq_has_rendered_hreflang,
AS_PERCENT(COUNTIF(wpt_bodies_info.has_rendered_hreflang), COUNT(0)) AS pct_has_rendered_hreflang,

# Both hreflangs
# COUNTIF(wpt_bodies_info.robots_has_both_hreflangs) AS freq_has_both_hreflangs,
AS_PERCENT(COUNTIF(wpt_bodies_info.has_both_hreflangs), COUNT(0)) AS pct_has_both_hreflangs
