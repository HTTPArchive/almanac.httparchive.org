...
SELECT
  ...
  COUNTIF(REGEXP_CONTAINS(hsts_header_val, r'(?i)max-age\s*=\s*[^0]')) / COUNTIF(hsts_header_val IS NOT NULL) AS pct_valid_max_age,
  COUNTIF(REGEXP_CONTAINS(hsts_header_val, r'(?i)max-age\s*=\s*0')) / COUNTIF(hsts_header_val IS NOT NULL) AS pct_zero_max_age,
