...
RETURNS ARRAY<STRUCT<name STRING, attribute STRING, value STRING, fetchpriority STRING>>
LANGUAGE js AS '''
...
try {
  ...
  return linkNodes.nodes.reduce((results, link) => {
    ...
    if (!hints.has(hint) || !link['fetchpriority']) {
      return results;
    }
...
FROM (
  ...
  FROM (
    SELECT
      ...
      hint.fetchpriority AS fetchpriority
    FROM
      `httparchive.all.pages`
    JOIN
