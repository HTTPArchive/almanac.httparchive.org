...
WITH adguard_trackers AS (
  ...
  SELECT
    client,
    NET.REG_DOMAIN(cnames.cname_domain) AS cname,
  ...
  ON ENDS_WITH(cnames.cname_domain, adguard_trackers.domain)
  LEFT JOIN whotracksme
  ON ENDS_WITH(cnames.cname_domain, whotracksme.domain)
  GROUP BY
    client,
    cname,
    adguard_known_cname,
