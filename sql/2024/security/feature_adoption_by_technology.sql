...
WITH
  ...
  SELECT
    ...
    COUNT(DISTINCT
    IF
      (STARTS_WITH(page, 'https'), page, NULL)) AS total_https_pages
  FROM
...
SELECT
  ...
  COUNT(DISTINCT
  IF
    (REGEXP_CONTAINS(respOtherHeaders, CONCAT('(?i)', headername, ' ')), url, NULL)) AS freq,
  SAFE_DIVIDE(COUNT(DISTINCT
    IF
      (REGEXP_CONTAINS(respOtherHeaders, CONCAT('(?i)', headername, ' ')), url, NULL)), total_pages_with_technology) AS pct,
  SAFE_DIVIDE(COUNT(DISTINCT
    IF
      (REGEXP_CONTAINS(respOtherHeaders, CONCAT('(?i)', headername, ' '))
        AND STARTS_WITH(url, 'https'), url, NULL)), total_https_pages) AS pct_https
FROM (
  SELECT
    client,
    technologies,
