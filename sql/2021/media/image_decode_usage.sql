...
SELECT
  ...
  COUNTIF(images_info.decode_lazy > 0) AS pages_with_decode_lazy,
  SAFE_DIVIDE(COUNTIF(images_info.decode_lazy > 0), COUNT(0)) AS pages_with_decode_lazy_pct,
  SUM(images_info.total) AS img_total,
  SUM(images_info.decode_lazy) AS imgs_with_decode_lazy,
  SAFE_DIVIDE(SUM(images_info.decode_lazy), SUM(images_info.total)) AS imgs_with_decode_lazy_pct
FROM (
  SELECT
