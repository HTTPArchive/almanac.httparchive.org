...
total_number_of_affected_accounts_per_data_class_and_date = defaultdict(
...
for breach in breaches_data:
    ...
    breach_data_classes = breach[
        "DataClasses"
    ]  # Type of data breached, e.g., email addresses
    for data_class in breach_data_classes:
        total_number_of_affected_accounts_per_data_class_and_date[data_class][
            breach_date
        ] += number_of_affected_accounts
    ...
    if is_breach_sensitive:
        total_number_of_affected_accounts_for_sensitive_breaches[
            breach_date
        ] += number_of_affected_accounts
...
with open("data_breaches_pwned_websites_per_class_and_date.csv", "w") as of1:
    ...
    with open("data_breaches_pwned_websites_per_class.csv", "w") as of2:
        ...
        for data_class in total_number_of_affected_accounts_per_data_class_and_date:
            for (
                breach_date
            ) in total_number_of_affected_accounts_per_data_class_and_date[data_class]:
                csvw_cd.writerow(
                    [
                        data_class,
                        breach_date,
                        total_number_of_affected_accounts_per_data_class_and_date[
                            data_class
                        ][breach_date],
                    ]
                )
