...
total_nb_affected_accounts_per_data_class_and_date = defaultdict(lambda: defaultdict(int))
total_nb_affected_accounts_for_sensitive_breaches = defaultdict(int)

for breach in breaches_data:
    nb_affected_accounts = breach["PwnCount"]
    ...
    for data_class in breach_data_classes:
        total_nb_affected_accounts_per_data_class_and_date[data_class][breach_date] += nb_affected_accounts
    ...
    if is_breach_sensitive:
        total_nb_affected_accounts_for_sensitive_breaches[breach_date] += nb_affected_accounts
...
with open("data_breaches_pwned_websites_sensitive_breaches_per_date.csv", "w") as of3:
    ...
    with open("data_breaches_pwned_websites_sensitive_breaches.csv", "w") as of4:
        ...
        for breach_date in total_nb_affected_accounts_for_sensitive_breaches:
            csvw_sbd.writerow([
                breach_date,
                total_nb_affected_accounts_for_sensitive_breaches[breach_date]
            ])
        csvw_sb.writerow([
            sum(total_nb_affected_accounts_for_sensitive_breaches.values())
        ])
