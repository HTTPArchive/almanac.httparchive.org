...
SELECT
  ...
  COUNTIF(uses_cache_control AND uses_expires) AS total_using_both_cache_control_and_expires,
  COUNTIF(NOT uses_cache_control AND NOT uses_expires) AS total_using_neither_cache_control_nor_expires,
  ...
  COUNTIF(uses_etag AND uses_last_modified) AS total_using_both_etag_and_last_modified,
  COUNTIF(NOT uses_etag AND NOT uses_last_modified) AS total_using_neither_etag_nor_last_modified,
  ...
  COUNTIF(uses_cache_control AND uses_expires) / COUNT(0) AS pct_using_both_cache_control_and_expires,
  COUNTIF(NOT uses_cache_control AND NOT uses_expires) / COUNT(0) AS pct_using_neither_cache_control_nor_expires,
  ...
  COUNTIF(uses_etag AND uses_last_modified) / COUNT(0) AS pct_using_both_etag_and_last_modified,
  COUNTIF(NOT uses_etag AND NOT uses_last_modified) / COUNT(0) AS pct_using_neither_etag_nor_last_modified
FROM (
  SELECT
    year,
    client,
