...
# capped to one hit per url to avoid having the results skewed by websites which dns-prefetch the same host many times (!)
    ...
    FROM (
        ...
        FROM (
          SELECT
            ...
            url,
            getResourceHintsHrefs(payload, "dns-prefetch") AS value
          FROM
