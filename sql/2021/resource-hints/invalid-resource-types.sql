...
RETURNS ARRAY<STRUCT<type STRING, num_occurences NUMERIC>> 
LANGUAGE js AS '''
try {
    ...
    // return JSON.stringify({ validTypesCnt, invalidTypesCnt, ...invalidTypes });
    // return { validTypesCnt, invalidTypesCnt, ...invalidTypes };
    var result = [
...
SELECT client, type, SUM(num_occurences) AS total_occurences
FROM (
  SELECT 
