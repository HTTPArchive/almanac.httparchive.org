SELECT
  ...
  APPROX_QUANTILES(xmlhttprequest_total, 1000)[
    OFFSET(percentile * 10)] AS xmlhttprequest_total,
  APPROX_QUANTILES(fetch_total, 1000)[
    OFFSET(percentile * 10)] AS fetch_total,
  APPROX_QUANTILES(beacon_total, 1000)[
    OFFSET(percentile * 10)] AS beacon_total
FROM (
    SELECT
      _TABLE_SUFFIX AS client,
      JSON_EXTRACT(JSON_EXTRACT_SCALAR(payload,
        '$._javascript'),
