...
SELECT
  ...
  COUNTIF(REGEXP_CONTAINS(cookie_value, r'(?i);.*max-age=.+')) AS count_max_age,
  COUNTIF(REGEXP_CONTAINS(cookie_value, r'(?i);.*max-age=.+')) / COUNT(0) AS pct_max_age,
  COUNTIF(REGEXP_CONTAINS(cookie_value, r'(?i);.*expires=.+')) AS count_expires,
  COUNTIF(REGEXP_CONTAINS(cookie_value, r'(?i);.*expires=.+')) / COUNT(0) AS pct_expires,
  COUNTIF(NOT(REGEXP_CONTAINS(cookie_value, r'(?i);.*max-age=.+') OR REGEXP_CONTAINS(cookie_value, r'(?i);.*expires=.+'))) AS count_session,
  COUNTIF(NOT(REGEXP_CONTAINS(cookie_value, r'(?i);.*max-age=.+') OR REGEXP_CONTAINS(cookie_value, r'(?i);.*expires=.+'))) / COUNT(0) AS pct_session,
