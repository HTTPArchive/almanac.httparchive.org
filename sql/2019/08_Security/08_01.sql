...
SELECT 
  ...
  ROUND(COUNT(0) / (SELECT COUNT(0) FROM `httparchive.almanac.requests_desktop_1k` WHERE JSON_EXTRACT_SCALAR(payload, '$._tls_version') IS NOT NULL), 2) pct_tls_version
FROM 
