...
FROM (SELECT _TABLE_SUFFIX, REGEXP_EXTRACT(respOtherHeaders, r'(?i)\W?strict-transport-security =([^,]+)') AS hsts FROM `httparchive.summary_requests.2019_07_01_*` WHERE firstHtml)
