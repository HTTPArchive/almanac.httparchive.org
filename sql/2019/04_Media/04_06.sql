...
SELECT 
     ...
     LENGTH(srcset) - LENGTH(REGEXP_REPLACE(srcset, ',', '')) +1 AS srcsetCOUNT,
     #this COUNTs the number of commas, AND adds one - to get the number of sizes
     LENGTH(sizes) - LENGTH(REGEXP_REPLACE(sizes, ',', '')) +1 AS sizesCOUNT, 
     #ratio of srcset to sizes
     (LENGTH(srcset) - LENGTH(REGEXP_REPLACE(srcset, ',', '')) +1)/(LENGTH(sizes) - LENGTH(REGEXP_REPLACE(sizes, ',', '')) +1) AS ratio
     ,
     LENGTH(src)/LENGTH(src) AS  srcCOUNT,
     LENGTH(alt)/LENGTH(alt) AS altCOUNT, 
     LENGTH(alt) AS altLENGTH,
     #double check that its working
     srcset, sizes, alt
FROM(

SELECT REGEXP_EXTRACT(flat_img,r'srcset=".*?"') AS srcset, 
	REGEXP_EXTRACT(flat_img,r'sizes=".*?"') AS sizes,
	REGEXP_EXTRACT(flat_img,r'src=".*?"') AS src, 
	REGEXP_EXTRACT(flat_img,r'alt=".*?"') AS alt
