...
FROM (
  SELECT
    ...
    IF(
      REGEX_CONTAINS(JSON_EXTRACT(report, '$.audits.offscreen-images.details.items'), ','),
      ARRAY_LENGTH(split(JSON_EXTRACT(report, '$.audits.offscreen-images.details.items'), ',')), 0
    ...
    IF(
      REGEX_CONTAINS(JSON_EXTRACT(report, '$.audits.uses-optimized-images.details.items'), ','),
      ARRAY_LENGTH(split(JSON_EXTRACT(report, '$.audits.uses-optimized-images.details.items'), ',')), 0
